...
the 'rootfs_ab' image handler: it reads the U-Boot 'rootfs_part' env var, \
writes the bundled rootfs image to the inactive slot via the built-in raw \
handler and flips rootfs_part on success. Loaded by SWUpdate at startup, \
which requires CONFIG_HANDLER_IN_LUA (enabled via the swupdate bbappend)."
